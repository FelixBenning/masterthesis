...
\end{lemma}
\begin{proof}
	The direction is irrelevant for the distance penalty term \(\|y-x\|^2\), and since
	\begin{align*}
		y-x = -\tfrac{r}{\|\nabla f(x)\|}\nabla f(x)
	\end{align*}	
	solves
	...
	\end{align*}
	as can be seen via the Cauchy-Schwarz inequality, we can reformulate our
...
this changes now. While Lipschitz continuity of the gradient provides an upper
...
\begin{proof}
	...
	its Lipschitz continuity. I.e. by the previous lemma (\ref{eq: min
	approximation}) we know that
	%
	\begin{align*}
		0 = \bergmanDiv{f}(x,x)
		\xeq{f\text{ convex}} \min_z\phi(z) \le \phi(y) - \tfrac{1}{2\ubound}\|\nabla \phi(y)\|^2.
	\end{align*}
	%
	...
	\end{align*}
	The second inequality follows by adding \(\bergmanDiv{f}(x,y)\) to
	\(\bergmanDiv{f}(y,x)\).
\end{proof}
\fxnote{
...
\begin{proof}
	...
	\begin{align*}
		...
		&\le \Delta_n(1 - \tfrac{\omega}{\|\weights_0-\minimum\}}\Delta_n)
	\end{align*}
	%
	Dividing both sides by \(\Delta_n\Delta_{n+1}\) results in
	%
	\begin{align*}
		\frac{1}{\Delta_{n+1}}
		&\ge \frac{1}{\Delta_n}
		+ \frac{\omega}{\|\weights_0-\minimum\|}\overbrace{\frac{\Delta_n}{\Delta_{n+1}}}^{\ge 1} \\
		&\lxge{\text{Induction}} \frac{1}{\Delta_0}
		+ \tfrac{\omega}{\|\weights_0-\minimum\|}(n+1)\\
	\end{align*}
\end{proof}
...
\begin{proof}[Proof {\parencite[Theorem 2.1.15]{nesterovLecturesConvexOptimization2018}}]
	\begin{align*}
		...
		+ \lr^2 \| \nabla\loss(\weights_n)\|^2
	\end{align*}
	Using \(\nabla\loss(\minimum)=0\) and Theorem~\ref{appdx-thm: technical
	strong convexity convergence proof component}, i.e.
 	\begin{align*}
