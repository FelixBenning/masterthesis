...
\begin{theorem}[\cite{nesterovLecturesConvexOptimization2018}]
	Let \(\loss\in\lipGradientSet{\ubound}\), then Gradient
...
\begin{theorem}\label{thm: convex function complexity bound}
	...
	\begin{align}
		\loss(\weights_n) - \inf_\weights \loss(\weights)
		&\ge \frac{\ubound \|\weights_0 - \weights^*\|^2}{16(n+1)^2} \\
		\|\weights_k -\weights^*\|^2 
		&\ge \frac13 \|\weights_0 - \weights^*\|^2
	\end{align}
	\end{subequations}
	where \(\weights^* = \arg\min_\weights \loss(\weights)\).
\end{theorem}
\begin{proof}
	...
	\begin{align*}
		...
		\right].
	\end{align*}
	Now we know that \(\weights^* = (1,\dots,1)^T\) achieves a loss of zero which is
	...
	\begin{align*}
		\|\weights^* - \weights_n\|^2
		&\ge \sum_{k=n+1}^d ((\weights^*)^{(k)}- \weights_n^{(k)})^2
		= d-(n+1) \\
		&= n = \tfrac{n}{2n+1} d
		\ge \tfrac13 \|\weights^* - \weights_0\|^2. 
	\end{align*}
	...
	\begin{align*}
		...
		=\frac{\ubound}{8(n+1)}
		\xeq{(\ref{eq: initial distance})} \frac{L\|\weights_0 - \weights^*\|^2}{8(n+1)d}\\
		&\lxge{(\ref{eq: w.l.o.g dim=2n+1})}
		\frac{L\|\weights_0 - \weights^*\|^2}{16(n+1)^2}
		\qedhere
	\end{align*}
\end{proof}
...
\begin{theorem}[{\cite[Theorem 2.1.13]{nesterovLecturesConvexOptimization2018}}]
	For any \(\weights_0\in\reals^\infty\) there exists
	...
	\begin{align}
		\|\weights_n - \weights^*\|
		&\ge \left(1-\frac2{1+\condition}\right)^n \|\weights_0 - \weights^*\| \\
		\loss(\weights_n) - \loss(\weights^*)
		&\ge \tfrac{\lbound}{2}
		\left(1-\frac2{1+\condition}\right)^{2n} \|\weights_0 - \weights^*\|^2
	\end{align}
	\end{subequations}
	where \(\weights^*\) is the unique minimum of \(\loss\).
\end{theorem}
\begin{proof}
