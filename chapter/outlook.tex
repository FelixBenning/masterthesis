...
\begin{align*}
	\weights_{n+1} := \arg\min_\lr \Loss(\weights_n - \lr d_{n+1})
\end{align*}
resulting in
\begin{align*}
	\weights_n = \arg\min\{ \Loss(\weights) : \weights\in\weights_0 + \mathcal{D}_{n-1}\}.
\end{align*}
...
\begin{proof}
	...
	H\Delta\weights_0\) and the induction step starts with
	\begin{align*}
		\nabla\Loss(\weights_{n+1})= H\Delta\weights_{n+1}
		&= H(\Delta\weights_n - \lr_n \nabla\Loss(\weights_n))\\
		&= \underbrace{\nabla\Loss(\weights_n)}_{\in \mathcal{D}_n} - \lr_n H
		\underbrace{\nabla\Loss(\weights_n)}_{\in \mathcal{D}_n}.
	\end{align*}
	To finish this step, we apply the induction assumption and notice that the
	second summand might now contain the vector \(H^{n+2}\Delta\weights_0\) if the
	coefficient of \(H^{n+1}\Delta\weights_0\) was not zero. Therefore
	\(\mathcal{D}_{n+1}\) is contained in the Krylov subspace. And due to the
	number of elements in the generator of the Krylov subspace it can not have
	higher rank than \(n+2\), and thus cannot be larger.
\end{proof}
This fact is useful, because it implies that \(H\mathcal{D}_{n-1}\) is contained
in \(\mathcal{D}_n\) to which \(\nabla\Loss(\weights_{n+1})\) was orthogonal.
Therefore \(\nabla\Loss(\weights_{n+1})\) is already \(H\)-orthogonal to
\(\mathcal{D}_{n-1}\) from the get go. In other words we already have
\begin{align*}
	\langle \nabla\Loss(\weights_{n+1}), d^{(k)} \rangle_H = 0 
	\qquad \forall k\le n-1.
\end{align*}
To construct a conjugate \(d^{(n+1)}\) we therefore only have to remove the
\(d^{(n)}\) component from our gradient! So we want to select
\begin{align*}
	d^{(n+1)} = \nabla\Loss(\weights_{n+1})
	- \frac{\langle \nabla\Loss(\weights_{n+1}), d^{(n)}\rangle_H}{\|d^{(n)}\|_H^2}d^{(n)}.
\end{align*}

\subsubsection{Evaluating the \(H\)-Dot-Product}

Now, we ``just'' have to find a way to evaluate this factor without actually
knowing the Hessian \(H\).
