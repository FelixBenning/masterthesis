...
But since we cannot obtain a gradient with respect to this norm, and do not
know \(H\) in general, it is difficult to actually work with this space.
The method that turns out to still work on this space, is the method of
conjugate directions.

\subsubsection{Conjugate Directions}

Let us take any (normed) direction \(v^{(1)}\) and complete it to an orthonormal
basis \(v^{(1)},\dots,v^{(\dimension)}\) on \((\reals^d, \|\cdot\|_H)\). Then we
can represent any vector \(\theta=\weights-\minimum\) as a linear combination of
this orthonormal basis
\begin{align*}
	\theta = \sum_{k=1}^\dimension \theta^{(k)} v^{(k)}.
\end{align*}
Let us now optimize over the learning rate for a move in direction \(v_1\)
\begin{align*}
	\weights_{n+1} = \weights_n + \lr_n v_1
	= (\theta_n^{(1)} + \lr_n)v^{(1)} + \sum_{k=2}^\dimension \theta_n^{(k)} v^{(k)}.
\end{align*}
Due to orthogonality we have
\begin{align*}
	\Loss(\weights_{n+1}) = \Loss(\minimum) + \sum_{k=1}^{\dimension} (\theta_{n+1}^{(k)})^2.
\end{align*}
And since most of the \(\theta^{(k)}_{n+1}\) stay constant no matter the learning
rate \(\lr_n\), the only one we actually optimize over is \((\theta^{(1)}_{n+1})^2\).
And we can make that coordinate zero with \(\lr_n=-\theta^{(1)}_n\).

So if we do a line search (optimize the loss over one direction, i.e. \(v^{(1)}\)),
then we know that we will never ever have to move in the direction \(v^{(1)}\)
again. The target lies orthogonal to \(v^{(1)}\). Note that this is generally
not the case for condition numbers other than zero\fxwarning{picture}.
So the trick is to select orthogonal search directions not with respect to the
euclidean scalar product, but rather orthogonal with respect to
\(\langle\cdot,\cdot\rangle_H\), i.e. ``conjugate'' to each other. As we pick
a conjugate second search direction, we can without loss of generality assume
that the second search direction is \(v^{(2)}\) otherwise we can just switch
to a new basis where this is true. Similar for the other dimensions. Therefore
we are going to remove one component in every step\footnote{
	This implies that this method is optimal in the linear span of search
	directions (compare with Assumption~\ref{assmpt: parameter in linear hull of
	gradients})!
}.

\subsubsection{Evaluation Troubles}
