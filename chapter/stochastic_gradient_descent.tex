...
do so far, is to minimize the empirical risk (\ref{eq: empirical risk})
\begin{align*}
...
actually allow for different models. E.g. chaining linear functions only results
in a linear function and therefore does not increase the expressiveness of our
hypothesis class. Nevertheless deep learning models should still be highly
over-parametrized according to this theory, and have no business generalizing to
unseen examples. Yet they seem to.
...
Here \(g\) can be, but needs not be, a gradient (we have not used that in
...
\end{align*}
But it does not worsen much, if the learning rate is small to begin with. A
learning rate of one on the other hand allows instant convergence while \ref{eq:
...
Another view to make this more intuitive is the analysis we did when selecting
...
Now we need to translate this intuition into actual mathematics. For that, we
define a new norm
\begin{align}
	\|X\|_{V,\infty}
...
to this new norm. As we are in a finite dimensional real vector space, the norms
are equivalent, more specifically we have
\begin{align*}
	\|X\|_{V,\infty}^2
...
\begin{proof}
	...
	Enacting this idea using the exponential function bound \(1-x\le\exp(-x)\), we
	get
	\begin{align}
		\nonumber
...
converged. Since we do not follow a straight gradient path anymore with
...
We will now follow a proof by \textcite[pp. 27-28]{bottouOptimizationMethodsLargeScale2018}
...
rather than to reduce them too quickly, which slows progress down below \(O(\tfrac1n)\).
...
decreasing distance from the optimum, we cannot use the same approach we used
...
In this setting we assume \(\Loss \in \lipGradientSet[0,0]{\lipConst}\).
...
So which one is better? Increasing the batch size, or reducing the size of the
...
term as hinted at in the proof of the theorem). But as we have seen in
...
logarithmically, i.e.\ increase the number of steps which decrease at rate
\(O(\tfrac1n)\) resulting in a logarithmic time increase 
\[
...
steps continues optimization as well as reduce variance, while batches
\emph{only} reduce variance.
...
\begin{theorem}[{\cite[Theorem 1.1, 1.2]{ankirchnerApproximatingStochasticGradient2021}}]
