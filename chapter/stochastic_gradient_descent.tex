...
\begin{theorem}\label{thm: optimal rates quadratic case}
...
As we argued leading up to this theorem, the bounds
...
\end{proof}
\begin{remark}\label{rem: rate is retained for larger eigenvalues}
...
We will now borrow an argument from \textcite[pp. 27-28]{bottouOptimizationMethodsLargeScale2018}
...
\begin{theorem}[Piecewise Constant Learning Rates]
...
In this setting we are going to assume \(\Loss \in \lipGradientSet[0,0]{\lipConst}\).
...
\begin{theorem}[Optimal Rates]\label{thm: optimal averaging rates}
...
Sure, if we moved into a convex area relatively quickly and stayed there for
...
So which one is better?  If we have a look at the term (\ref{eq: mean of
...
following section (\ref{subsec: variance reduction}) for constant learning
rates, halving them is ever so slightly worse than simply reducing the variance
...
the same improvements on the variance as increasing the batch size incurs.

Note that instead of running GD for the same ``time'' in Gradient Flow and
reducing the discretization size we would actually increase this ``time''
logarithmically (i.e. harmonic sum of learning rates) if we increase the number
of steps in the diminishing-learning-rate case. This effect can be best seen
in Theorem~\ref{thm: convergence rates for diminishing lr schedules} where
increasing the number of steps improves the ``GD part'' as well as the variance
reduction part, while decreasing the variance only has (the same) effect on the
variance.
...
So in the transient phase, a larger batch size only requires more computation
time (which might be hidden by parallelization as batch learning is easier to
parallelize than SGD) and more samples. If we assume independent identically
distributed data this means we have to stop training much earlier as we have
used up our sample set or risk overfitting.
...
size of \(m\). If we use too little learning rates during batch learning, then
this is similar to reducing the learning rate too fast in the diminishing case
as we are using up our resources more quickly without making full use of them.
In both cases we would therefore get a reduced convergence rate.

Our solution in Section~\ref{sec: simplified learning rate schedules} was
using piecewise constant learning rates. But as we have found, in
Subsection~\ref{subsec: variance reduction}, halving the learning rate is
ever so slightly worse than doubling the batch size. So the idea by
\textcite{smithDonDecayLearning2018}, of doubling the batch size (instead of
halving the learning rate) whenever progress stalls, is probably optimal.

In the general convex case where we apply averaging on the other hand, SGD
without batches is the clear winner. Only if \(\lipConst^2\) is negligible
...
Batch learning is finally a good idea, if we have a virtually infinite amount of
supply of data (i.e. we do not have to worry about overfitting), and we do not
feel the cost of batches due to parallelization. In that case the computational
speed stays the same whether we use batches or not, and we do not have to worry
running out of data, so we might as well use them.
