...
\begin{align*}
	...
	&=
	\underbrace{\Loss(\weights) - \Loss(\minimum^{(\sample)})}_{
	\le \sup_{\weights}|\Loss_\sample(\weights)-\Loss(\weights)|
	}
	{\scriptstyle
	+ \Loss_\sample(\weights) - \Loss_\sample(\minimum^{(\sample)})
	+ \underbrace{
		\scriptstyle
	...
	\\
	&\le \underbrace{\Loss_\sample(\weights) - \Loss_\sample(\minimum^{(\sample)})}_{
