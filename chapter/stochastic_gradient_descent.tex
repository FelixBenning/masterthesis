...
the VC-dimension. Although VC dimensions try to remove ``duplicates'', i.e.
...
and avoid taking the supremum over all possible parameters for all \(\weights\)
but rather only for the weights that our algorithm would actually pick. E.g. for
an algorithm \(\algo\) \textcite{hardtTrainFasterGeneralize2016} use the upper
bound
\begin{align*}
...
the (VC-)dimension of parameters, as they only depend on the amount of data and
...
do not have time to overfit. Now the advantage of these types of bounds is that
we can reuse our data as much as we want and fully optimize with regard to
\(\Loss_\sample\) without any concern for our sample size. And if we use
gradient descent on \(\Loss_\sample\) we do not even have another choice.
Stochastic Gradient Descent provides that choice. Instead of the ``full
gradient'' \(\nabla\Loss(\weights)\) we are going to use the stochastic
gradient
%
\begin{align*}
...
}. This will allow us to formulate convergence results which do not require
splitting the error into an optimization and generalization error as we do not
optimize with regard to \(\Loss_\sample\) but rather with regard to \(\Loss\)
directly and thus do not have a generalization error to deal with. But these
results also require an infinite supply of iid samples from \(\dist\) which
we generally do not have in practice. So after using up all of our examples
reusing them implies optimizing with regard to \(\Loss_\sample\) and no longer
with regard to \(\Loss\). This would force us to use these triangle inequality
upper bounds. Which (as of writing) are often so much worse, that even when the
number of iterations go to infinity, our upper bound is worse than if we
had stopped the iteration at the number of samples \(\sampleSize\) and used the
bounds without the triangle inequality\footnote{
	...
	\begin{align*}
		...
		\underbrace{\sqrt{\frac{\sampleSize+2n}{\sampleSize}}}_{\to \sqrt{2} \quad (n\to\infty)}
	\end{align*}
...
generalizing minimum already. And if that is also \emph{a} minimum of \(\Loss_\sample\)
...
well on \(\Loss_\sample\) this seems very plausible. Empirically it does in fact
...
Now if one wants to minimize \(\Loss\) then full gradients are not possible in
the first place, but for \(\Loss_\sample\) one might compare convergence rates
and deem GD on \(\nabla\Loss_\sample\) more appropriate than SGD. But while it
...
generalization. We will build intuition for this using mini-batches. Where we
