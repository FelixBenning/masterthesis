...
\begin{lemma}
	...
	\end{align*}
	This motivates why we are interested in \(\Gamma_0^n\to 0\). A sufficient
	condition is
	\begin{align*}
...
By definition \(z_{n+1}\) is the unique minimum of \(\Phi_{n+1}\) so we can find
it with
\begin{align*}
...
\begin{algorithm}
	...
	\(v_0\leftarrow \weights_0\);
	\(n\leftarrow 0\)\;
	\While{not converged}{
		\label{line: pick gamma}
		Select \(\gamma_n\in(0,1)\) such that
		\begin{algomathdisplay}
			\ubound\gamma_n^2 = \gamma_n\lbound+(1-\gamma_n)\lbound_n
		\end{algomathdisplay}
		%
		\(\lbound_{n+1}\leftarrow \gamma_n\lbound+(1-\gamma_n)\lbound_n\)\;

		\(y_{n+1}\leftarrow 
		\displaystyle
		\frac{\lbound_{n+1}\weights_n + \lbound_n\gamma_n z_n}{\gamma_n\lbound + \lbound_n}\)\;
		\label{line: definition of y_{n+1}}

		Select \(\weights_{n+1}\) such that 
		\tcp*[f]{e.g. \(\weights_{n+1}=y_{n+1}-\tfrac1\ubound\nabla\Loss(y_{n+1})\)}
		\begin{algomathdisplay}
			\Loss(\weights_{n+1}) \le \Loss(y_{n+1}) - \tfrac{1}{2\ubound}\|\nabla\Loss(y_{n+1})\|^2
		\end{algomathdisplay}\label{line: selecting next position}
		%
		\(z_{n+1}\leftarrow \tfrac{1}{\lbound_{n+1}}
			\left[{\scriptstyle \gamma_n\lbound} y_{n+1} + {\scriptstyle (1-\gamma_n)\lbound_n} z_n
			- {\scriptstyle \gamma_n}\nabla\Loss(y_{n+1})\right]
		\)
...
\begin{lemma}\label{lem: streamline general schema of optimal methods}
	...
	\begin{align}
		...
		z_{n+1}
		&= \weights_n + \tfrac1{\gamma_n}(\weights_{n+1}-\weights_n)\\
		y_{n+1}
	...
	\begin{align}
		\momCoeff_n
		= \frac{\lbound_n \gamma_n(1-\gamma_{n-1})}{\gamma_{n-1}(\gamma_n\lbound +\lbound_n)}
		= \frac{\gamma_{n-1}(1-\gamma_{n-1})}{\gamma_{n-1}^2 + \gamma_n}
	\end{align}
\end{lemma}
\begin{proof}
	\fxnote{appendix?}
	...
	\begin{align*}
		\lbound_n z_n
		= \tfrac{1}{\gamma_n} [(\gamma_n\lbound + \lbound_n)y_{n+1} - \lbound_{n+1}\weights_n]
	\end{align*}
	...
	\begin{align*}
		...
		&= \tfrac{1}{\lbound_{n+1}}
		\left[
			{ \gamma_n\lbound} y_{n+1}
			+ \tfrac{(1-\gamma_n)}{\gamma_n}
			[(\gamma_n\lbound + \lbound_n)y_{n+1} - \lbound_{n+1}\weights_n]
			- { \gamma_n}\nabla\Loss(y_{n+1})
		\right]\\
		&=\tfrac{1}{\lbound_{n+1}} \underbrace{\Big[
			\underbrace{\gamma_n\lbound + (1-\gamma_n)\lbound}_{=\lbound}
			+ \tfrac{1-\gamma_n}{\gamma_n}\lbound_n
		\Big]}_{
			=\lbound_{n+1}/\gamma_n
		}y_{n+1}
		-\tfrac{1-\gamma_n}{\gamma_n}\weights_n
		-\underbrace{\tfrac{\gamma_n}{\lbound_{n+1}}}_{
			=\frac{1}{\ubound\gamma_n} \mathrlap{\ (L\gamma_n^2 = \lbound_{n+1})}
		}\nabla\Loss(y_{n+1})\\
		&= \weights_n + \tfrac{1}{\gamma_n}(y_{n+1} - \weights_n)
		- \tfrac{1}{\ubound\gamma_n}\nabla\Loss(y_{n+1})\\
		&\lxeq{(\ref{eq-lem-assmpt: weigth recursion})}
		\weights_n + \tfrac{1}{\gamma_n}(\weights_{n+1}-\weights_n)
	\end{align*}
	Now we are going to rewrite \(y_{n+1}\)
	\begin{align*}
		y_{n+1}
		&= \frac{
			\overbrace{[\gamma_n \lbound + (1-\gamma_n)\lbound_n]}^{\lbound_{n+1}}\weights_n
			+ \lbound_n \gamma_n z_n
		}{\gamma_n\lbound +\lbound_n}
		=\weights_n
		+ \frac{\lbound_n \gamma_n (z_n - \weights_n)}{\gamma_n\lbound +\lbound_n},
	\end{align*}
	which allows us to apply our previous result (\ref{eq: z_n recursion}) to the difference
	\begin{align*}
		z_n - \weights_n
		= \left(\tfrac1{\gamma_{n-1}} -1\right)(\weights_n-\weights_{n-1}),
	\end{align*}
	...
	\begin{align*}
		...
		(\weights_n-\weights_{n-1}).
	\end{align*}
	Now we just use the identity \(\gamma_n\lbound = \ubound\gamma_n^2 -
	(1-\gamma_n)\lbound_n\) from the definition of \(\gamma_n\) to simplify our
	momentum coefficient
	\begin{align*}
		\momCoeff_n
		&= \frac{\lbound_n \gamma_n(1-\gamma_{n-1})}{
			\gamma_{n-1}([\ubound\gamma_n^2 - (\cancel{1}-\gamma_n)\lbound_n] +\cancel{\lbound_n})
		}
		= \frac{\lbound_n\cancel{\gamma_n}(1-\gamma_{n-1})}{
			\gamma_{n-1}\cancel{\gamma_n}(\ubound\gamma_n + \underbrace{\lbound_n}_{=\ubound\gamma_{n-1}^2})
		}\\
		&\lxeq{\bcancel{\ubound\gamma_{n-1}}} \frac{\gamma_{n-1}(1-\gamma_{n-1})}{\gamma_n + \gamma_{n-1}^2}
		\qedhere
	\end{align*}
\end{proof}

With the streamlined recursion from Lemma~\ref{lem: streamline general schema of optimal methods}
we only need \(\lbound_n\) for the recursion of \(\gamma_n\) itself at this
...
\begin{align}\label{eq: morphing speed recursion using condition}
	\gamma_n^2
	= \gamma_n \condition^{-1} + (1-\gamma_n)\condition_n^{-1}
	= \condition^{-1}_{n+1}
\end{align}
By defining \(\gamma_{-1}:=1/\sqrt{\condition_0}\) we can ensure
\(\gamma_{n-1}^2 = \condition_n^{-1}\). Applying the p-q formula and discarding
the negative option results in the explicit recursion
\begin{align*}
	\gamma_n
	= \tfrac12 \left[
		-(\gamma_{n-1}^2 - \condition^{-1})
		+ \sqrt{(\gamma_{n-1}^2 - \condition^{-1})^2 + 4 \gamma_{n-1}^2}
	\right].
\end{align*}
...
\begin{algorithm}
	...
	\(\gamma_{-1} \leftarrow 1/\sqrt{\condition_0}\)
	\tcp*{
		\(\gamma_{n-1}^2=\condition_n^{-1}\) in (\ref{eq: morphing speed recursion using condition})
		makes \(\condition_n\) obsolete
	}
	\(\weights_{-1} \leftarrow \weights_0\)\;
	\(n\leftarrow 0\)\;
	\While{not converged}{
		\(\gamma_n \leftarrow \tfrac12 \left[
		(\condition^{-1} - \gamma_{n-1}^2)
		+ \sqrt{(\condition^{-1} - \gamma_{n-1}^2)^2 + 4 \gamma_{n-1}^2}
		\right]\)\;
		\tcp{\(=1/\sqrt{\condition}\) if \(\condition_0=\condition\)}

		\(\momCoeff_n \leftarrow
		\displaystyle
		\frac{\gamma_{n-1}(1-\gamma_{n-1})}{\gamma_n + \gamma_{n-1}^2}\)
...
\begin{figure}[h]
	...
	\caption{
		...
		plotted against \(\gamma_{n-1}\) and \(\hat{\momCoeff}\) corresponds
		to \(\hat{\gamma}\).
		%
		...
		\(\Gamma_0^n=\prod_{k=0}^{n-1}(1-\gamma_k)\).
		%
