...
This Lemma allows us to formulate the simplified algorithm
...
\begin{algorithm}
	...
	\While{not converged}{
		\uIf{\(n=0\)}{
			Select \(\gamma_0\in(0,1)\) such that
			\tcp*[f]{Alternative: \(\gamma_0\) is input}
			\begin{algomathdisplay}
				\ubound\gamma_0^2 = \gamma_0\lbound+(1-\gamma_0)\lbound_0
			\end{algomathdisplay}
		}
		\Else{
			Select \(\gamma_n\in(0,1)\) such that
			\tcp*[f]{\(\lbound_n=\ubound\gamma_{n-1}^2\)}
			\begin{algomathdisplay}
	...
	\caption{Dynamic Nesterov Momentum}
\end{algorithm}

Since we get a fixed \(\hat{\gamma}=\tfrac1{\sqrt{\condition}}\) for
