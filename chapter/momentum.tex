...
If our friction \(\friction\) is low enough\footnote{
...
\begin{theorem}[\cite{qianMomentumTermGradient1999}]
	...
	\end{enumerate}
	See also Figure~\ref{fig: annotated heavy ball rates} for a visualization of
	these cases.
\end{theorem}
\begin{proof}
...
learning rate and eigenvalue products \(\lrSq\hesseEV\) which allows for larger
learning rates. As can be seen very well in Figure~\ref{fig: annotated heavy
ball rates}, the wider attraction area allows for moving out the largest
eigenvalues to the right with larger eigenvalues shifting the smaller
...
\begin{proof}
	...
	complex case. Now the question is what happens if we decrease it. Now by our
...
This convergence rate is in fact \emph{equal} to our lower bound  in
...
Heavy Ball Momentum, optimal learning rates are no use if we cannot generalize
them.
...
in mind that our upper bound is a quadratic function which is therefore
...
is therefore not smaller than the minimum of \(\phi_1=\Phi_1\) and since \(\phi_1\)
...
% 	\end{align*} 
% 	where we denote the sorted (increasing) eigenvalues of the Hessian
...
% 	is just a linear combination of gradients, we cannot achieve anything better
