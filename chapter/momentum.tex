...
strongest descend which is mostly to the opposite side of the ravine and only slightly
...
\begin{definition}[Heavy Ball Method]
	...
	obtain gradient descend again. This is of course an artifact of our
	discretization since \(\lr\to0\) would never allow \(\momCoeff\) to be zero
	in the limit. But actual implementations often use this
	(\(\momCoeff,\lrSq\))-parametrization and thus treat gradient descend as a
	special case.
\end{definition}
%
...
gradient descend on the other hand the contraction factor is \(|1-\lr_*\hesseEV_i|\)
...
Theorem~\ref{thm: convex function GD loss upper bound} for gradient descend 
...
descend convergence rate multiplied by a factor we could make smaller than one.
...
range just like in the gradient descend. And this is in fact not the optimal
...
To prove convergence for gradient descend we proved that gradient descend
...
\fxnote{backtracking/armijo's condition?}
The Lipschitz constant could be estimated with the fact that Gradient Decent
stops converging if the learning rate exceeds \(2/\ubound\). Similarly
Nesterov's Momentum stops converging if the learning rate is just a bit higher
than \(1/\ubound\) (cf. Figure~\ref{fig: annotated nesterov rates}). Since
the gradient is also Lipschitz continuous for higher constants, convergence
does not break if we guess \(\ubound\) too high. But the convergence rate
will take a hit accordingly. So generally we would want to increase our
learning rate as much as possible and see when it stops converging and then
back off a bit.

With the condition number we could just be conservative and set
...
\end{align}
is a descend approximation for the \(\momCoeff_n\) progression when \(\condition^{-1}=0\).
...
discretization is similar given that Gradient Decent converges for \(\lr \in
