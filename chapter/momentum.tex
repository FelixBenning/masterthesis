...
us consider
\begin{align*}
	\Phi_1 = \gamma_0 \phi_0 + (1-\gamma_0)\Phi_0
\end{align*}
For \(\gamma_0=1\) we would have a lower bound, but if \(\nabla\Loss(\weights_n)\)
does not happen to be zero, the tangent point \(y_0\) is sitting on a slope and
therefore is not smaller than the minimum of \(\phi_0=\Phi_1\) and since \(\phi_0\)
...
\begin{align*}
	\sum_{k=0}^n \hat{\gamma}_k \phi_k(x)
	+ \left(1-\sum_{k=0}^n \hat{\gamma}_k\right) \Phi_0(x)
\end{align*}
...
\begin{align}\label{eq: estimating sequence recursion}
	\Phi_{n+1}(x) = \gamma_n \phi_n(x) + (1-\gamma_n)\Phi_n(x)
\end{align}
...
\begin{align}
	\nonumber
	\Phi_{n+1}(\weights)
	&= \sum_{k=0}^n \Gamma_{k+1}^n\gamma_k \phi_k(\weights)
...
\begin{lemma}
	...
	then we have
	\begin{align*}
		\Loss(\weights_{n+1}) - \Loss(\weights_*)
...
\begin{proof}
	...
	second statement:
	\begin{align*}
		\Gamma_0^n &= \prod_{i=0}^n (1-\gamma_i)
...
\begin{align*}
	\Phi_{n}(\weights) = \underline{\Phi}_{n} + \tfrac{\mu_{n}}{2}\|\weights - z_n\|^2
\end{align*}
where our convexity parameter \(\lbound_n\) morphs from \(\lbound_0\) to
\(\lbound\)
\begin{align*}
	\lbound_{n+1} =(1-\Gamma_0^n)\lbound + \Gamma_0^n \lbound_0
	= \gamma_n \lbound_n + (1-\gamma_n)\lbound_0
\end{align*}
and \(z_n\) is the minimum. We can therefore write our minimal value as
\begin{align*}
	\underline{\Phi}_{n+1}
	&= \Phi_{n+1}(y_n) - \tfrac{\mu_{n}}{2}\|y_n - z_{n+1}\|^2 \\
	&= (1-\gamma_n)\underbrace{\Phi_n(y_n)}_{
		=\underline{\Phi}_{n} + \tfrac{\mu_{n}}{2}\|y_n - z_n\|^2
	} + \gamma_n \underbrace{\phi_n(y_n)}_{
		=\Loss(y_n)
	} -  \tfrac{\mu_{n}}{2}\|y_n - z_n\|^2
\end{align*}
