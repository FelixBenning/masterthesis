...
\begin{figure}[h]
	...
	\caption{
		...
		morphing speed equation using condition}) results in equality assuming
		\(\lbound_0\ge\lbound\).  If we start out with \(\condition_0=\condition\)
		we have \(\gamma_n=\hat{\gamma}=1/\sqrt{\condition}\)
		for all \(n\). In any case \(\hat{\gamma}\) acts as an attraction point.
		...
		%
		But increasing \(\lbound_0\) (i.e. \(\condition_0^{-1}\))
...
\begin{theorem}
	...
	\(\weights_0\) and some \(\lbound_0\) (equivalently \(\condition_0\) or
	\(\gamma_0\)) let \(\weights_n\) be selected via Algorithm~\ref{algo: dynamic
	nesterov momentum}, then for \(\Gamma^n := \Gamma_1^n =
...
\begin{proof}
	...
	\(\lbound_0=\lbound\) in (\ref{eq: convergence rate for lbound_0=lbound}),
