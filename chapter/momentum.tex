...
\begin{align*}
	\sum_{k=1}^n \hat{\gamma}_k \phi_k(x)
	+ \hat{\gamma}_0 \Phi_0(x)
\end{align*}
...
\begin{align}\label{eq: estimating sequence recursion}
	\Phi_{n+1}(x) = \gamma_n \phi_{n+1}(x) + (1-\gamma_n)\Phi_n(x)
\end{align}
...
holds for all \(k<n\), we can therefore write our minimal value as
\begin{align*}
	\underline{\Phi}_{n+1}
...
\end{align*}
Now you might recall that \(y_n\) are our intermediate "information collection"
points which then define iterate as
\begin{align*}
	\weights_n = y_n - \lrSq \nabla\Loss(y_n)
\end{align*}
...
\begin{align*}
	\Loss(y_n) - \tfrac{1}{2\ubound}\|\nabla\Loss(y_n)\|^2 \ge \Loss(\weights_n)
\end{align*}
...
express it in relation to \(z_{n-1}\). Now we know that \(z_n\) is the unique
minimum of \(\Phi_n\) so we can find it with
\begin{align*}
	0\xeq{!}\Phi'_n(z_n)
	= \gamma_{n-1}\underbrace{\phi_n'(z_n)}_{
...
\begin{align*}
	\overbrace{\lbound_n}^{
		=\mathrlap{\gamma_{n-1}\lbound + (1-\gamma_{n-1})\lbound_{n-1}}
	}z_n
	= \gamma_{n-1}\lbound y_n + (1-\gamma_{n-1})\lbound_{n-1}z_{n-1}
	- \gamma_{n-1}\nabla\Loss(y_n).
\end{align*}
Subtracting \(\lbound_n y_n\) results in
\begin{align*}
	\lbound_n(z_n-y_n)
	= (1-\gamma_{n-1})\lbound_{n-1}(z_{n-1}-y_n)
	- \gamma_{n-1}\nabla\Loss(y_n).
\end{align*}
Therefore
\begin{align*}
	...
	\begin{aligned}
		=&(1-\gamma_{n-1})^2\lbound_{n-1}^2\|z_{n-1}-y_n\| \\
		&- 2(1-\gamma_{n-1})\lbound_{n-1}\gamma_{n-1}\langle
		\nabla\Loss(y_n), y_n-z_{n-1}\rangle \\
		&+ \gamma_{n-1}^2\|\nabla\Loss(y_n)\|^2
	\end{aligned}
	}
\end{align*}
