...
to entertain the general case with general \(\lbound_0\). So let us summarize
...
\begin{align*}
	\momCoeff = \frac{1-1/\sqrt{\condition}}{1+1/\sqrt{\condition}}
\end{align*}
...
\begin{algorithm}
	...
	\tcp*{\(\gamma_{n-1}^2=\condition_n^{-1}\) in (\ref{eq: morphing speed recursion using condition})
	makes \(\condition_n\) obsolete
	}
	\(n\leftarrow 0\)\;
	\While{not converged}{
		Select \(\gamma_n\in(0,1)\) such that
		\tcp*[f]{\(=1/\sqrt{\condition}\) if \(\condition_0=\condition\)}
		\begin{algomathdisplay}
			\gamma_n^2 = \gamma_n \condition^{-1} + (1-\gamma_n) \gamma_{n-1}^2
		\end{algomathdisplay}
