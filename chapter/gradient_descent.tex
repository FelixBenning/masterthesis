...
\end{align}
%
This ODE is known as ``Gradient Flow''. If you are familiar with Lyapunov
functions you will not be surprised by the next argument:
%
...
represent a saddle point (cf. Figure~\ref{fig: visualize saddlepoint gd}), or
...
(Figure~\ref{fig: visualize saddlepoint gd}) is a possible explanation for the
...
In the following we are going to assume convexity.
%
\begin{figure}[h]
	\centering
	\def\svgwidth{1\textwidth}
...
\begin{definition}[Convexity]\label{def: convexity}
	...
	definition is equivalent to non-emptyness of the set of subgradients
...
\begin{remark}
	...
	\(\hesseEV_d/\hesseEV_1 =\condition\), then you will not be surprised to see us 
	struggle to get a convergence rate with only non-negative eigenvalues in 
	the remaining section. As eigenvalues close or equal to zero will blow up this
...
We still need a replacement for the upper bound of our second derivative. Since
if the second derivative was unbounded then we could have arbitrarily high
eigenvalues \(\hesseEV_i\) which would make it impossible to select a learning rate
small enough that we can guarantee
\begin{align*}
	-1 \le 1-\lr\hesseEV_i \qquad \forall i=1,\dots, \dimension
\end{align*}
Using the orthonormal basis of eigenvectors \((v_i)_i\) of
a (symmetric) matrix \(A\) to represent a point \(x\in\reals^\dimension\)
\begin{align*}
...
absolute eigenvalue using \(x=\stdBasis_d\). This implies that for positive
eigenvalues 
%
\begin{align}
