...
The upper bound requires some discussion. First notice that the operator norm is
equal to the largest absolute eigenvalue
\begin{align*}
	\|A\| := \sup_{\|x\| =1} \|Ax\|
...
\section{Discussing Assumptions}
...
is in fact a minimum. This means that on a gaussian random field critical points
...
points of critical points found at a certain loss levels of a loss function
...
that this issue does not matter. Further theoretical justification is
provided by \textcite{choromanskaLossSurfacesMultilayer2015}.

\textcite{garipovLossSurfacesMode2018} find in an empirical study that local
minima can be connected by a simple path (e.g. linear spline with just one
fee knot) with near-constant loss. In other words the local minima do not only
...
methods like the Newton-Raphson Method, which has much better convergence
...
``center'' of a quadratic function. This is nice if this center is a minimum,
which convexity guarantees, but it is not so nice if the center is a saddle point
or maximum. Additionally the method becomes unstable (impossible) for small 
(zero) eigenvalues as the hesse matrix is then hard to numerically invert (no
longer invertible).
Additional to these fundamental problems, the storage of the Hesse matrix requires
\(O(\dimension^2)\) space already and its inversion takes \(O(\dimension^3)\)
time which is infeasible for high dimensional data, especially since gradient
...
convexity is a special case. Considering (\ref{eq: gradient integral}) it is
perhaps not too surprising that a lower bound on the gradient for a given
distance we want to cover allows for convergence proofs. But while convexity
...
since this generalization highlights the attributes of (strong)-convexity we
actually need, it is well worth a complementary read. Their convergence proofs
are very easy to follow.

\subsection{Discussing Lipschitz Gradients}

There are two somewhat convincing reasons why there is probably not much more
...
\end{align}
%
The distance penalty can be understood in view of Lemma~\ref{lem-appendix: lipschitz and bounded derivative}
...
\end{lemma}
\begin{proof}
	The direction is irrelevant for the distance penalty term \(\|\theta-\weights\|^2\), and
	\begin{align*}
		\theta-\weights = -\tfrac{r}{\|\nabla \Loss(\weights)\|}\nabla \Loss(\weights)
	\end{align*}	
	solves
...
\begin{proof}
	...
	\begin{align}
		...
		- 2\lr\underbrace{
			...
			\ge \tfrac{1}\ubound \|\nabla \Loss (\weights_n) - \nabla\Loss(\minimum)\|^2
			\mathrlap{\quad \text{using (\ref{eq: bregmanDiv lower bound b})}}
		} + \lr^2\|\nabla \Loss(\weights_n)\|^2
		\\
...
%
%
Using this new assumption we get our linear
