...
\end{align*}
which implies convergence of \(\Loss(\weights(t))\). But not necessarily a
convergent \(\weights(t)\) and not necessarily convergence to \(\inf_\weights
\Loss(\weights)\).
...
the second order Taylor approximation \(T_2\Loss\) of the loss \(\Loss\)
%
\begin{align*}
...
So instead of non-negative eigenvalues we can demand ``convexity'' which does
not require the existence of a hesse matrix but is equivalent to its positive
definiteness if it exists.
%
...
\(\nabla \Loss\). And these two formulations are equivalent under convexity
%
...
As a bounded hesse matrix \(\nabla^2\Loss \precsim \ubound\) implies a limited
rate of change of the gradient, the upper bound we have
...
\end{align}
%
can also be understood as a distance penalty, penalizing the fact that we do
not have enough information about the loss if we move farther away from our
current point. Our limit on the change of the gradient
...
Now we need to lower bound our gradient norm to ensure our loss decreases at
...
\begin{proof}
	...
	Here we need to bring in convexity again. With that we will prove in
	Lemma~\ref{lem: bermanDiv lower bound} that
	\begin{align*}
...
our loss function is really flat, then the loss difference to the minimum might
already be really small even if we are still far away.

Strong Convexity prevents that by requiring a minimum of curvature. So we can
provide a lower bound of the gradient using the weight difference directly
\begin{align}
	\nonumber
...
\begin{proof}[Proof {\parencite[Theorem 2.1.15]{nesterovLecturesConvexOptimization2018}}]
 ...
 \begin{align*}
	...
	on the Bregman Divergence from Lemma~\ref{lem: bermanDiv lower bound}, now
	we could also use the lower bound from the strong convexity property. But
	we can not use both which seems like we are loosing sharpness in our bound.
	And indeed this bound can be improved to be
 	\begin{align*}
 ...
 \begin{align*}
	...
	1}) using induction. Note that we could also use our lower bound on the
	gradient, but this does not represent a significant improvement especially
	for the optimal rates where we max out the learning rate making the
	term on the right zero. (\ref{eq: gd strong convexity convergence rate 2})
	follows from our upper bound on the loss using weights, cf. (\ref{eq: weights bound loss}) 
	\begin{align*}
		\Loss(\weights_n) - \Loss(\minimum)
...
Another pain point is the fact that we just end up in \emph{some} convex area in
...
negative. But the lower the loss at the critical point, the likelier it is
...
Lipschitz continuity represents an upper bound on the Bregman divergence
\begin{align*}
	\bregmanDiv{f}(y,x)
	= f(y) - f(x) - \langle\nabla f(x), y-x\rangle
	\lxle{(\ref{eq: lin approx + distance penalty notion})}
...
\begin{align*}
	...
	&\le \min_y\{f(x) + \langle f(x), y-x\rangle +\tfrac{\ubound}{1} \|y-x\|^2\}.
\end{align*}
The basic idea of ``Mirror Decent'' is: If we had a
better upper bound on the Bregman divergence, then we could also improve on
...
existence, uniqueness and maybe most importantly stability of ODEs. While these
...
\begin{align}\label{eq: bounded gradient integral}
	\int_{t_0}^\infty \|\nabla \Loss(\weights(s))\|^2 ds
	&\le \Loss(\weights(t_0)) - \lim_{t\to\infty} \Loss(\weights(t)) \\
...
large or small which would prevent convergence of the gradient decent iteration as
the difference between \(\weights_{n+1}\) and \(\weights_n\) is proportional to
the gradient.

Note that convergence of the gradient does not yet ensure that \(\weights(t)\)
...
In the discrete case we get a similar relationship. Instead of the integral
we get a sum using Lemma~\ref{lem: smallest upper bound}
%
\begin{align*}
...
It is difficult to formulate an example in finite space, but for
%
\begin{align*}
