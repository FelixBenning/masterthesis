...
To build some intuition what leads to convergence of \(\weights_n\)
(\(\weights(t)\)) let us consider more simple cases. For the second order Taylor
approximation \(T_2\Loss\) of the loss \(\Loss\)
%
\begin{align*}
...
The strongest type of convergence is ``convergence of the weights''
\begin{align*}
	\|\weights_n - \minimum\| \to 0,
\end{align*}
which implies ``convergence of the loss'' at a squared rate.
\begin{lemma}[Weight- implies Loss-Convergence]
\label{lem: weight implies loss convergence}
	For a loss \(\Loss\) with Lipschitz continuos gradient, we have
	\begin{align*}
...
\end{lemma}
\begin{proof}
	The claim follows from Lemma~\ref{lem: Lipschitz Gradient
	implies taylor inequality} using \(\nabla\Loss(\minimum)=0\):
	\begin{align*}
		\Loss(\weights) - \Loss(\minimum)
	...
	\end{align*}
\end{proof}
Even weaker is ``convergence of the gradient'', which is implied by convergence of
the loss
\begin{lemma}[Loss- implies Gradient-Convergence]
	\label{lem: loss implies gradient convergence}
	For a loss \(\Loss\) with Lipschitz continuos gradient, we have
	\begin{align*}
		\|\nabla\Loss(\weights)-\overbrace{\nabla\Loss(\minimum)}^{=0}\|^2
		\le 2\ubound (\Loss(\weights) - \Loss(\minimum)).
	\end{align*}
	More generally, we have for all \(\lr\in(0, \tfrac{2}{\ubound})\)
	\begin{align*}
		\|\nabla\Loss(\weights)\|^2
		\le \frac{\Loss(\weights) - \Loss(\weights - \lr\nabla\Loss(\weights))}{
			\lr(1-\tfrac{\ubound}2 \lr)
		}
		\le \frac{\Loss(\weights) - \Loss(\minimum)}{
			\lr(1-\tfrac{\ubound}2 \lr)
		}.
	\end{align*}
\end{lemma}
\begin{proof}
	The first claim is a special case of the second with \(\lr=1/\ubound\). For
	the second claim we use Lemma~\ref{lem: Lipschitz Gradient implies taylor inequality}
	\begin{align*}
		|\Loss(\weights-\lr\nabla\Loss(\weights)) - \Loss(\weights)
		- \langle \nabla \Loss(\weights), -\lr \nabla \Loss(\weights)\rangle|
		\le \|\nabla\Loss(\weights)\|^2,
	\end{align*}
	and use the fact that in general \(x\le |x|\), to obtain
	\begin{align*}
		\Loss(\weights-\lr\nabla\Loss(\weights))
		&\le \Loss(\weights)
		+ \langle \nabla \Loss(\weights), -\lr \nabla \Loss(\weights)\rangle
		+ \tfrac{\ubound}{2}\|\lr\nabla \Loss(\weights)\|^2\\
		&=\Loss(\weights) - \lr(1-\tfrac{\ubound}2\lr)\|\nabla\Loss(\weights)\|^2.
	\end{align*}
	Reordering the terms results in our claim.
\end{proof}

The weakest form of convergence is virtually always a given

\begin{corollary}
	For a Loss function with Lipschitz gradient and finite lower bound
	(i.e. \(\inf_{\weights}\Loss(\weights) >-\infty\)) Gradient Descent induces
	convergence of the gradient. The convergence rate is at least \(o(1/n)\).
\end{corollary}
\begin{proof}
	...
	\begin{align*}
		...
		&\xle{\text{Lem. }\ref{lem: loss implies gradient convergence}}
...
But while we can ensure convergence to a critical point, we can not ensure
that this is a (global) minimum. For that we need to assume convexity.
...
\begin{proof}
	...
	\begin{align*}
		...
		&\xle{\text{Lem. }\ref{lem: weight implies loss convergence}}
...
\begin{align*}
	\|\nabla\Loss(\weights)\|\le \lbound\|\weights -\minimum\|
\end{align*}
allows us to prove convergence of the weights with convergence of the gradients.
More specifically we can bound our weight iteration
\begin{align*}
	\|\weights_{n+1}-\minimum\|^2
...
\begin{proof}[Proof {\parencite[Theorem 2.1.15]{nesterovLecturesConvexOptimization2018}}]
 ...
 \begin{align*}
	...
	cf. Lemma~\ref{lem: weight implies loss convergence} 
	\begin{align*}
		\Loss(\weights_n) - \Loss(\minimum)
...
using weights (Lemma~\ref{lem: weight implies loss convergence}).
