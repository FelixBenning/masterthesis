...
Taking the derivative of the representation (\ref{eq: paraboloid approximation
...
Since we cannot guarantee convergence to a (global) minimum as convergence of
...
\begin{proof}
	...
	Utilizing convexity again we will prove in Lemma~\ref{lem: bregmanDiv lower bound} that
	\begin{align*}
		\langle\nabla\Loss(\weights) - \nabla\Loss(\minimum), \weights-\minimum\rangle
		\ge \tfrac1{\ubound} \|\nabla\Loss(\weights) - \nabla\Loss(\minimum)\|^2.
	\end{align*}
...
from the minimum.
%
\begin{lemma}[{\cite[Theorem~2.1.5]{nesterovLecturesConvexOptimization2018}}]\label{lem: bregmanDiv lower bound}
...
This difficulty is the reason why we skip this starting phase and only
...
lower bound on the gradient for a given loss difference we want to cover allows for
...
The assumption of strong convexity leads some to suggest second order
...
the ``Conjugate Gradient (CG) Method'' \parencite[for an introduction see
...
some critical point for general \(C^1\) functions.\fxnote{present result here?}
...
\begin{align*}
	...
	\underbrace{\frac{\weights^{(i)}}{\weights^{(i-1)}}}_{=q} +1.
\end{align*}
...
convex coloring problem c}). This is why we are going to formulate the following
