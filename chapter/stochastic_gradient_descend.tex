...
we could keep the benefits on the variance constant while improving the
...
very good for small \(x\) and our learning rate is decreasing we can probably
rule out that the detour over the exponential function and logarithm messed
with our asymptotic rate (although it might be possible to get rid of the
constant \(c\) from the integral), but in general the fault probably does not
lie with our bounds but rather with our selection of the learning rate
\(\lr_n = \tfrac{1}{\hesseEV_\dimension(n+1)}\).

In the next section we are going to find locally optimal learning rates (i.e.
learning rates which maximize the decrease from \(n\) to \(n+1\)).
...
implies
\begin{align*}
	\lim_{n\to\infty} n\E\|\Weights_n - \minimum\|^2
	= \stdBound^2 \left(\frac{\ubound+\lbound}{\ubound\lbound}\right)^2
\end{align*}
Notice how that implies
\begin{align*}
	\E\|\Weights_n - \minimum\|^2 \in O(\stdBound^2/n)
\end{align*}
...
\begin{align*}
	\lim_{n\to\infty} n \lr_n = \frac{\ubound\lbound}{\ubound+\lbound}.
\end{align*}
...
theory that no batches are preferable.
