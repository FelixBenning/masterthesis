...
σ_2(αλ, β) = 0.5*(1+β-αλ - sqrt((1+β-αλ)^2 - 4*β))

# ╔═╡ 6ad1f3c9-3fcb-4644-860f-8b3d40f0c89e
function max_abs_sigma(αλ, β)
	if ((1+β-αλ)^2 - 4*β) > 0 # real case
		return 1+β > αλ ? σ_1(αλ, β) : -σ_2(αλ, β)
	end # complex case
	return sqrt(β)
end

# ╔═╡ f425ff9f-5ed9-4c0b-a9a7-a43b56a74534
max_abs_sigma(x) = max_abs_sigma(x[1],x[2])

# ╔═╡ 15306a8d-5491-4a7a-924d-db8484e5baa8
n_max_abs_sigma(αλ, β) = max_abs_sigma(αλ, β*(1-αλ))

# ╔═╡ b511a60d-9f59-4686-af3d-34e4a5da6951
n_max_abs_sigma(x) = n_max_abs_sigma(x[1],x[2])

# ╔═╡ 1e7559dc-caab-4529-b8e5-d97121406314
md"Show Labels 
$(@bind show_labels PlutoUI.CheckBox())
Precision 
...
begin
	...
	Plots.savefig(heavy_ball, "heavy_ball_rates.svg")
	md"## Heavy Ball Momentum"
end

# ╔═╡ 39181ac5-76a3-4049-8775-3177b88ee83f
begin
	annotated_hb = deepcopy(heavy_ball)
	if show_labels
		plot!(
			annotated_hb, [1+beta_min, 1], x->x-1, 
			color=:black, linestyle=:dot, linewidth=2, 
			label="\$\\alpha\\lambda=1+\\beta\$",
			legend=:bottomright
		)
		plot!(
